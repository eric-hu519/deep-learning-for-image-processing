...
def plot_loss_and_lr(train_loss, val_loss, learning_rate, path):
    try:
        x = list(range(len(train_loss)))
        fig, ax1 = plt.subplots(1, 1)
        ax1.plot(x, train_loss, 'r', label='train_loss')
        ax1.plot(x, val_loss, 'b', label='val_loss')
        ax1.set_xlabel("step")
        ax1.set_ylabel("loss")
        ax1.set_title("Train/Val Loss and lr")
        plt.legend(loc='best')
